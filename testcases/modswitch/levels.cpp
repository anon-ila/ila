...
int main(void)
{
    ...
    */
    ...
    cout << "Computation without modulus switching." << endl;
    print_line(__LINE__);
    cout << "Compute the 8th power." << endl;
    cout << "    + Noise budget fresh:                   " << decryptor.invariant_noise_budget(encry_le) << " bits"
         << endl;
    evaluator.square_inplace(encry_le);
    evaluator.relinearize_inplace(encry_le, relin_keys);
    cout << "    + Noise budget of the 2nd power:         " << decryptor.invariant_noise_budget(encry_le) << " bits"
         << endl;
    evaluator.square_inplace(encry_le);
    evaluator.relinearize_inplace(encry_le, relin_keys);
    cout << "    + Noise budget of the 4th power:         " << decryptor.invariant_noise_budget(encry_le) << " bits"
         << endl;
    ...
    evaluator.square_inplace(encry_le);
    evaluator.relinearize_inplace(encry_le, relin_keys);
    cout << "    + Noise budget of the 8th power:         " << decryptor.invariant_noise_budget(encry_le) << " bits"
         << endl;
    //decryptor.decrypt(encry_le, plain);
